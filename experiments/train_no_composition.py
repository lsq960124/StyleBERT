...
from model import BertMultiNoCompositionModel
...
ddef evaluate(model,data_loader,best_acc6,history_acc6):
    model.eval()
    losses = []
    y_trues = []
    y_predicts = []
...
model = BertMultiNoCompositionModel(hp).to(device)
print("BertMultiNoCompositionModel model have {} paramerters in total".format(sum(x.numel() for x in model.parameters())))
