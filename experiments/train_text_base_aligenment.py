...
from model import BertMultiTextBaseAligenmentModel
...
model = BertMultiTextBaseAligenmentModel(hp).to(device)
print("BertMultiTextBaseAligenmentModel model have {} paramerters in total".format(sum(x.numel() for x in model.parameters())))
...
for epoch in range(hp.epochs):
    ...
    print(f"train    loss: {np.mean(losses):.4f} \t learn rate {hp.learn_rate:.6f} \t wa: {wa:.4f} \t ua: {ua:.4f} \t")
